...
base_url = 'https://kodepos.posindonesia.co.id/kelurahandesalist'
params = {
    'recperpage': 100,
    'search': '',
}
...
with open('kelurahan_desa.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)

    # Write the header row
    writer.writerow(['No.', 'Kode', 'Nama', 'Propinsi','Kabupaten','Kecamatan','Jenis'])
    ...
    while True:
        ...
        rows = soup.select('#tbl_kelurahandesalist tbody tr')
        ...
        for idx,row in enumerate(rows):
            ...
            writer.writerow([no, kode, nama,provinsi,kabupaten,kecamatan,jenis])
